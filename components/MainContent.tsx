...
import { FileText, ExternalLink, ChevronRight, FolderOpen, ArrowLeft, Cloud, Folder, Presentation, FileSpreadsheet, Search, XCircle } from 'lucide-react';
...
export const MainContent: React.FC<MainContentProps> = ({ 
  ...
  // View 0: Search Results
  if (searchQuery) {
      const results = MOCK_MATERIALS.filter(material => 
      ...
      return (
        <div className="flex-1 p-8 overflow-y-auto h-screen">
            <header className="mb-8 flex items-center gap-2 text-slate-500">
                <Search className="w-5 h-5" />
                <h2 className="text-xl font-semibold">{t.searchResults}: <span className="text-slate-900">"{searchQuery}"</span></h2>
            </header>
                ...
                <div className="grid gap-0 divide-y divide-slate-100">
                    ...
                    <div key={material.id} className="p-5 flex flex-col sm:flex-row sm:items-center justify-between hover:bg-slate-50 transition-colors group">
                        ...
                        <div className={`p-3 rounded-lg transition-colors ${
                            ...
                            {isDriveConnected ? <Cloud className="w-6 h-6" /> : getMaterialIcon(material.type)}
                        </div>
                        <div>
                        ...
                        className={`flex items-center justify-center gap-2 px-4 py-2 bg-white border rounded-lg text-sm font-medium transition-all shadow-sm hover:shadow ${
                        isDriveConnected 
